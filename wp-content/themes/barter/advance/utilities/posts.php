...
if(!function_exists( 'barter_post_meta_data' )){
    ...
    function barter_post_meta_data() {
        echo barter_get_post_meta_data(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
}
            ...
            <div class="about-author comment"<?php barter_schema_args('author_box'); ?>>
                <div class="comment-body">
                    <?php $author_ID = get_the_author_meta( 'ID' );
                        echo '<a href="'.esc_url( get_author_posts_url($author_ID) ).'" class="avatar"'.barter_get_schema_args('url').'>'.get_avatar( $author_ID, 90 ).'</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                        echo '<strong class="comment-author"><span class="author-name" '.barter_get_schema_args('name').'>'.get_the_author().'</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            ...
            if($is_prev || $is_next){
                ...
                if($is_prev){
                    ...
                    echo '<a href="'.esc_url( get_permalink($id) ).'" class="item prev">'
                         ...
                         .'<span class="image">'.barter_make_post_image( $id, $image_sizes ).'</span>' // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                         .'</a>';
                }
                if($is_next){
                    $id = $next_post->ID;
                    ...
                    if(post_password_required()){
                        ...
                        <div class="formatter">
                            ...
                            <div class="real-content">
                                ...
                                <?php echo barter_password_form(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped  ?> 
                            </div>
                        </div>
                        <?php
                    }
...
if(!function_exists('barter_schema_args')){
    ...
    function barter_schema_args( $for ) {
        echo barter_get_schema_args( $for ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    }
}
